...
ipcMain.on('checkProjectBalances', (event, args) => {
    ...
    function callback (resp, projectInfo) {
        ...
        if (resp.type === "data") {
            ...
            logger.error('checkProjectBalances: ' + resp.msg)
        }
    ...
    global.availableProjects.list.forEach(function(project, projectKey) {
        smartcashapi.checkBalance({address: project.address.publicKey, projectID: project.id, projectIndex: projectKey}, callback)
    })
})
...
ipcMain.on('getClaimedFundsInfo', (event, args) => {
    ...
    if (args === undefined) {
        ...
        global.availableProjects.list.forEach(function(project, projectKey) {
            project.recvAddrs.forEach(function(address, addrKey) {
                smartcashapi.checkBalance(address.publicKey, {addrAmt: project.addrAmt}, callback)
                totalAddrs++
            })
        })
    }
        ...
        project.recvAddrs.forEach(function(address, addrKey) {
            smartcashapi.checkBalance(address.publicKey, {projectID: args.projectID, projectIndex: index, addrAmt: project.addrAmt}, callback)
            totalAddrs++
        })
    }
})
